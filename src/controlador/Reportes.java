...
InterFacturacion f= new InterFacturacion();        
    ...
    public void reportesMotocicletas(){
        ...
        try {
            ...
            try {
                Connection cx=ConectionDB.conectar();
                PreparedStatement st=cx.prepareStatement("select p.id_motocicleta, p.nombre, p.cilindraje, p.cantidad, p.precio, p.descripcion,"
                        + " p.porcentajeIva, c.descripcion as categoria, p.estado "
                        + "from motocicleta As p, categoria As c where p.id_categoria=c.id_categoria;");
                ResultSet rs=st.executeQuery();
    ...
    public void reportesVentas(){
        ...
        try {
           ...
           PdfWriter.getInstance(document, new FileOutputStream(f.guardarComo()));
            ...
            document.add(parrafo);
            //Ajustar columnas
            float [] columnasWidths=new float[]{3,9,4,5,3};
            ...
            try {
                Connection cx=ConectionDB.conectar();
                PreparedStatement st=cx.prepareStatement("select cv.id_venta as Id, concat(c.nombre,' ',c.apellido) as Cliente, "
                        + "cv.valorPagar as Total, cv.fechaVenta as Fecha, cv.estado "
                        + "from cabecera_venta as cv, cliente as c where cv.id_cliente=c.id_cliente;");
                ResultSet rs=st.executeQuery();
